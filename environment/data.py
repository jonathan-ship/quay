...
def import_data(info_path, scenario_path):
    df_quay = pd.read_excel(info_path, sheet_name="안벽 기준정보", header=1)
    df_work = pd.read_excel(info_path, sheet_name="작업 기준정보", header=1)
    df_score = pd.read_excel(info_path, sheet_name="안벽작업 기준정보", header=1)
    ...
    df_ship = pd.read_excel(scenario_path, sheet_name="호선정보")
    df_work_fix = pd.read_excel(scenario_path, sheet_name="고정작업정보")
